...
const copy = {
  en: {
    ...
    title: "Reach the driving test prepared.",
    intro: "LicencePath checks eligibility and evidence early, recovers uncertain payments and keeps the next owner visible.",
  ...
  hi: {
    ...
    title: "ड्राइविंग टेस्ट के लिए तैयार पहुंचें।",
    intro: "LicencePath पात्रता और दस्तावेज पहले जांचता है, अनिश्चित भुगतान सुलझाता है और अगली जिम्मेदारी साफ रखता है।",
...
export function AboutContent({ language }: { language: AboutLanguage }) {
  ...
  const imageAlt = language === "en"
    ? "Hands arranging synthetic driving-test preparation sheets on a desk"
    : "मेज पर नकली ड्राइविंग टेस्ट तैयारी के कागज व्यवस्थित करते हाथ";

  return (
    <main className="about-page" lang={language}>
      <div className="about-wrap">
        <div className="about-nav">
          ...
          <Link className="back-link" href={t.switchHref}>
        ...
        <header className="about-hero">
          <div className="about-hero-copy">
            <span className="mock-chip">{t.chip}</span>
            <h1>{t.title}</h1>
            <p>{t.intro}</p>
            <div className="about-hero-actions">
              ...
              <a className="source-link" href="https://github.com/Businessagents/Sarathi-Portal" rel="noreferrer" target="_blank">
                <GithubLogo aria-hidden="true" size={19} weight="bold" />
                {t.sourceCta}
              </a>
            </div>
          </div>
          <figure className="about-hero-visual">
            <Image
              ...
              src="/images/licencepath-preparation.png"
              width={1600}
            />
          </figure>
        </header>

        <section className="problem-grid" aria-labelledby="problem-heading">
          <div><h2 id="problem-heading">{t.problemTitle}</h2><p>{t.problem}</p></div>
          <div><h2>{t.changedTitle}</h2><p>{t.changed}</p></div>
        </section>

        <section className="evidence-section" aria-labelledby="evidence-heading">
          <div><h2 id="evidence-heading">{t.evidenceTitle}</h2></div>
          <ul>{t.evidence.map((item) => <li key={item}>{item}</li>)}</ul>
        </section>

        <section className="comparison-section" aria-labelledby="comparison-heading">
          <h2 id="comparison-heading">{t.compareTitle}</h2>
          <div className="comparison-table" role="table">
            {t.compare.map(([current, proposed], index) => (
              <div className={index === 0 ? "comparison-head" : "comparison-row"} key={current} role="row">
                <span role={index === 0 ? "columnheader" : "cell"}>{current}</span>
                <span role={index === 0 ? "columnheader" : "cell"}>{proposed}</span>
              </div>
            ))}
          </div>
          <small>{language === "en" ? "This comparison is a research hypothesis, not an audited statement about every State portal." : "यह तुलना शोध परिकल्पना है, हर राज्य पोर्टल का ऑडिट दावा नहीं।"}</small>
        </section>

        <section className="portal-overview" aria-labelledby="about-licencepath-heading">
          <div className="overview-copy"><h2 id="about-licencepath-heading">{t.aboutTitle}</h2><p>{t.about}</p></div>
          <dl className="portal-facts">{t.facts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
        </section>

        <section aria-labelledby="boundary-heading">
          <h2 id="boundary-heading">{t.boundaryTitle}</h2>
          <div className="boundary-grid">
            {t.boundary.map(({ title, items }, index) => {
              const Icon = boundaryIcons[index];
              return <article className="boundary-block" key={title}><Icon aria-hidden="true" size={28} weight="duotone" /><h3>{title}</h3><ul>{items.map((item) => <li key={item}>{item}</li>)}</ul></article>;
            })}
          </div>
        </section>

        <section className="scale-section" aria-labelledby="scale-heading">
          <div className="scale-copy"><LockKey aria-hidden="true" size={34} weight="duotone" /><h2 id="scale-heading">{t.scaleTitle}</h2><p>{t.scale}</p></div>
          <ol className="architecture-flow" aria-label={t.architectureLabel}>{t.architecture.map((item) => <li key={item}>{item}</li>)}</ol>
        </section>

        <section className="governance-section" aria-labelledby="governance-heading">
          <h2 id="governance-heading">{t.governanceTitle}</h2>
          <p>{t.governance}</p>
          <dl>
            <div><dt>{language === "en" ? "Current pack" : "मौजूदा पैक"}</dt><dd>delhi-demo-2026-08-28-v2</dd></div>
            <div><dt>{language === "en" ? "Approval" : "मंजूरी"}</dt><dd>{language === "en" ? "Illustrative, unverified" : "उदाहरण, असत्यापित"}</dd></div>
            <div><dt>{language === "en" ? "Production owner" : "असल जिम्मेदार"}</dt><dd>{language === "en" ? "Relevant State Transport Department" : "संबंधित राज्य परिवहन विभाग"}</dd></div>
          </dl>
        </section>

        <section className="principles" aria-labelledby="principles-heading">
          <h2 id="principles-heading">{t.safetyTitle}</h2>
          <div>{t.safety.map(([label, text]) => <p key={label}><strong>{label}</strong> {text}</p>)}</div>
        </section>
